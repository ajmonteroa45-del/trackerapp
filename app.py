...
with st.sidebar:
    ...
    lang = st.selectbox("Idioma / Language", options=["Español","English"], key="lang_selector")
    alias = st.text_input("Alias / Nombre", value="", key="alias_input")
    pin = st.text_input("PIN (4-6 dígitos)", type="password", key="pin_input")
    users = load_users()
    col1, col2 = st.columns(2)

    if col1.button("Entrar" if lang=="Español" else "Login", key="login_button"):
        if not alias or not pin:
            st.sidebar.error("Alias y PIN requeridos" if lang=="Español" else "Alias and PIN required")
        else:
            u = load_users()
            if alias in u and hash_pin(pin) == u[alias]["pin_hash"]:
    ...
    if col2.button("Registrar" if lang=="Español" else "Register", key="register_button"):
        if not alias or not pin:
            st.sidebar.error("Alias y PIN requeridos" if lang=="Español" else "Alias and PIN required")
        else:
            u = load_users()
...
with st.form("trips_form"):
    cantidad = st.number_input("¿Cuántos viajes vas a registrar ahora?", min_value=1, step=1, value=1, key="cantidad_input")
    rows = []
    ...
    submitted = st.form_submit_button("Agregar viajes" if lang=="Español" else "Add trips", key="submit_trips")
    if submitted:
        csv_path = ensure_user_csv(alias)
...
with st.form("extras_form"):
    extras_total = st.number_input("Total viajes extra (S/)", min_value=0.0, format="%.2f", value=0.0, key="extras_input")
    gastos_varios = st.number_input("Gastos varios (S/)", min_value=0.0, format="%.2f", value=0.0, key="gastos_input")
    combustible = st.number_input("Combustible (S/)", min_value=0.0, format="%.2f", value=0.0, key="comb_input")
    km = st.number_input("Kilometraje recorrido (km)", min_value=0.0, format="%.1f", value=0.0, key="km_input")
    submit_extras = st.form_submit_button("Guardar extras" if lang=="Español" else "Save extras", key="submit_extras")
    ...
    if submit_extras:
        ...
        summary = {"date": date.today().isoformat(), "total_viajes": int(total_viajes),
                   "bonos": bonos, "extras_total": extras_total, "gastos": gastos_varios,
                   "combustible": combustible, "total_neto": total_neto}
        ...
        st.download_button("Descargar imagen (PNG)" if lang=="Español" else "Download image (PNG)",
                           data=buf, file_name=f"balance_{date.today().isoformat()}.png", mime="image/png")

# Botón para descargar CSV (fuera del form)
if os.path.exists(csvp):
    with open(csvp, "rb") as f:
        st.download_button("Descargar CSV (todos tus registros)" if lang=="Español" else "Download CSV (all your records)",
                           data=f, file_name=os.path.basename(csvp), mime="text/csv", key="download_csv")

# Logout
if st.button("Cerrar sesión" if lang=="Español" else "Logout", key="logout_button"):
